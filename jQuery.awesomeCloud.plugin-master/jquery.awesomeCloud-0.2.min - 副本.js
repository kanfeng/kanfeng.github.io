 ...
 */
(function(e){"use strict";function r(e,t){
	this.bctx=null;
	this.bgPixel=null;
	this.ctx=null;
	...
	this.words=[];
	this.linkTable=[];
	this.match=null;return this}

var t="awesomeCloud",n={size:{grid:8,factor:0,normalize:true},
color:{background:"rgba(255,255,255,0)",
start:"#20f",end:"#e00"},
options:{color:"gradient",rotationRatio:.3,printMultiplier:1,sort:"highest"},
font:"Helvetica, Arial, sans-serif",shape:"circle"};

e.fn.awesomeCloud=function(i,s){
	if(typeof i==="object"){s=i}
	else if(typeof i==="string"){
		var o=this.data("_"+t);
		if(o){if(n[i]!==undefined){if(s!==undefined){o.settings[i]=s;return true}else{return o.settings[i]}}else{return false}}
		else{return false}
		}
		s=e.extend(true,{},n,s||{});
		return this.each(function(){
			var n=e(this),
			i=jQuery.extend(true,{},s),
			o=new r(i,n);
			o.create();
			n.data("_"+t,o)})
			};
...
r.prototype={create:function(){
	...
	this.settings.weightFactor=function(e){return e*n.settings.size.factor};
	...
	switch(this.settings.shape){
		case"square":this.settings.shape=function(e){var t=(e+Math.PI/4)%(2*Math.PI/4);return 1/(Math.cos(t)+Math.sin(t))};
		break;
		case"diamond":this.settings.shape=function(e){var t=e%(2*Math.PI/4);return 1/(Math.cos(t)+Math.sin(t))};
		break;
		case"x":this.settings.shape=function(e){var t=e%(2*Math.PI/4);return 1/(Math.cos(t)+Math.sin(t)-2*Math.PI/4)};
		break;case"triangle":this.settings.shape=function(e){var t=(e+Math.PI*3/2)%(2*Math.PI/3);return 1/(Math.cos(t)+Math.sqrt(3)*Math.sin(t))};
		break;case"triangle-forward":this.settings.shape=function(e){var t=e%(2*Math.PI/3);return 1/(Math.cos(t)+Math.sqrt(3)*Math.sin(t))};
		break;case"pentagon":this.settings.shape=function(e){var t=(e+.955)%(2*Math.PI/5);return 1/(Math.cos(t)+.726543*Math.sin(t))};
		break;case"star":this.settings.shape=function(e){var t=(e+.955)%(2*Math.PI/10);if((e+.955)%(2*Math.PI/5)-2*Math.PI/10>=0){return 1/(Math.cos(2*Math.PI/10-t)+3.07768*Math.sin(2*Math.PI/10-t))}else{return 1/(Math.cos(t)+3.07768*Math.sin(t))}};
		break;case"circle":this.settings.shape=function(e){return 1};
		break;default:this.settings.shape=function(e){return 1};
		break}
	
	this.size={left:this.container.offset().left,
	...
	if(this.settings.ellipticity>2){this.settings.ellipticity=2}
	if(this.settings.ellipticity<.2){this.settings.ellipticity=.2}
	...
	this.container.children().each(function(t,r){
		...
		s+=o;if(!n.settings.weight.lowest){n.settings.weight.lowest=o}
		if(!n.settings.weight.highest){n.settings.weight.highest=o}
		if(o<n.settings.weight.lowest){n.settings.weight.lowest=o}
		if(o>n.settings.weight.highest){n.settings.weight.highest=o}
		n.settings.weight.average=s/(t+1);
		e(this).css("display","none");
		if(e(this).has("a").length===0){y=e(this).html()}
		else{var i=e(this).children(":first");
		      m=i.attr("href");
			  g=i.attr("target");
			  y=i.html()}
	...
	if(this.settings.size.normalize===true){this.words.sort(function(e,t){return e[1]-t[1]});for(r=0;r<this.words.length;r++){if(a===null){a=this.words[r][1]}else{if(this.words[r][1]-a>this.settings.weight.average){this.words[r][1]-=(this.words[r][1]-a)/(this.settings.weight.average*.38)+a}}}}
	
	this.words.sort(function(e,t){if(n.settings.options.sort==="random"){return.5-Math.random()}else if(n.settings.options.sort==="lowest"){return e[1]-t[1]}else{return t[1]-e[1]}});
	
	if(this.settings.size.factor===parseInt(0,10)){this.settings.size.factor=1;i=t+"SizeTest";f=false;l=0;c=.1;h=0;p=0;d=0;v=0;b=Math.min(this.size.width,this.size.height);w=this.createCanvas({id:i,width:b,height:b,left:0,top:0});for(r=0;r<this.words.length;r++){w.font=this.settings.weightFactor(this.words[r][1])+"px "+this.settings.font;v=w.measureText(this.words[r][0]).width;if(v>h){h=v;p=this.words[r]}}while(!f){l=this.settings.weightFactor(p[1]);w.font=l.toString(10)+"px "+this.settings.font;v=w.measureText(p[0]).width;if(v>b*.95){this.settings.size.factor-=c}else if(v<b*.9){this.settings.size.factor+=c}else{f=true}d+=1;if(d>1e4){f=true}}this.destroyCanvas(i);this.settings.size.factor-=c}
	
	this.settings.color.increment={r:(this.settings.color.end.r-this.settings.color.start.r)/this.settings.range,g:(this.settings.color.end.g-this.settings.color.start.g)/this.settings.range,
	...
	this.grid=[];i=t+this.container.attr("id");
	...
	if(typeof this.settings.options.color!=="function"&&this.settings.options.color.substr(0,6)!=="random"&&this.settings.options.color.substr(0,8)!=="gradient"){
		...
		r=4;
		while(r--){if(Math.abs(E[r]-this.bgPixel[r])>10){this.diffChannel=r;break}}
	}
		else{this.diffChannel=NaN}
		this.destroyCanvas(u);
		S=this.ngx;while(S--){this.grid[S]=[];x=this.ngy;while(x--){this.grid[S][x]=true}}
		...
		r=0;
		window.setImmediate(function T(){if(r>=n.words.length){return}n.putWord(n.words[r][0],n.words[r][1],n.words[r][2],n.words[r][3]);r+=1;window.setImmediate(T)});
		n.allDone(i);
		return true},
		
		allDone:function(t){
			var n=this,r=document.getElementById(t);
			e("#"+t).width(this.size.screenWidth);e("#"+t).height(this.size.screenHeight);
			e("#"+t).css("display","block");
			e("#"+t).css("visibility","visible");
			r.addEventListener("mousemove",function(t){var i=0,s=0;if(t.layerX||t.layerX===0){i=t.layerX;s=t.layerY}i-=r.offsetLeft;i+=e(r).position().left;i=Math.floor(i*n.settings.options.printMultiplier);s-=r.offsetTop;s+=e(r).position().top;s=Math.floor(s*n.settings.options.printMultiplier);n.match=null;for(var o=0,u=n.linkTable.length;o<u;o++){var a=n.linkTable[o];if(i>=a.x&&i<=a.x+a.width&&s>=a.y&&s<=a.y+a.height){n.match=a}}if(n.match!==null){document.body.style.cursor="pointer"}else{document.body.style.cursor=""}},false);
			r.addEventListener("click",function(e){if(n.match!==null){if(n.match.target){window.open(n.match.link,n.match.target)}else{window.location=n.match.link}}},false)
		},
		
		minimumFontSize:function(){
			var e=t+"FontTest",n=this.createCanvas({id:e,width:50,height:50,left:0,top:0}),r=20,i,s;
			while(r){n.font=r.toString(10)+"px sans-serif";if(n.measureText("Ｗ").width===i&&n.measureText("m").width===s){this.destroyCanvas(e);return(r+1)/2}i=n.measureText("Ｗ").width;s=n.measureText("m").width;r-=1}
			this.destroyCanvas(e);
			return 0},
		createCanvas:function(t){
			var n=t.id,r,i=e("body");
			if(t.parent!==undefined){i=t.parent}
		...
		destroyCanvas:function(t){e("#"+t).remove()},
		...
		putWord:function(e,n,r,i){
			...
			if(u<=this.settings.minSize){return false}
			this.ctx.font=u.toString(10)+"px "+this.settings.font;
			if(o){a=this.ctx.measureText(e).width;f=Math.max(u,this.ctx.measureText("m").width,this.ctx.measureText("Ｗ").width);if(/[Jgpqy]/.test(e)){f*=3/2}f+=Math.floor(u/6);a+=Math.floor(u/6)
				}else{f=this.ctx.measureText(e).width;a=Math.max(u,this.ctx.measureText("m").width,this.ctx.measureText("Ｗ").width);if(/[Jgpqy]/.test(e)){a*=3/2}a+=Math.floor(u/6);f+=Math.floor(u/6)}
			f=Math.ceil(f);
			a=Math.ceil(a);
...
(jQuery);Array.prototype.shuffle=function(){
	...
	n;e=parseInt(Math.random()*n,10),t=this[--n],this[n]=this[e],this[e]=t);
	return this
	};
...
if(!window.setImmediate){
	window.setImmediate=function(){
		...
		return window.msSetImmediate||window.webkitSetImmediate||window.mozSetImmediate||window.oSetImmediate||function(){
			if(window.postMessage&&window.addEventListener){var e=[],t=-1,n=-1,r="zero-timeout-message",i=function(t){e.push(t);window.postMessage(r,"*");return++n},s=function(n){if(n.data===r){n.stopPropagation();if(e.length>0){var i=e.shift();i();t+=1}}},o;window.addEventListener("message",s,true);window.clearImmediate=function(r){if(typeof r!=="number"||r>n){return}o=r-t-1;e[o]=function(){}};return i}
		}()||function(e){window.setTimeout(e,0)}
	}()
}
...
if(!window.clearImmediate){
	window.clearImmediate=function(){
		"use strict";
		return window.msClearImmediate||window.webkitClearImmediate||window.mozClearImmediate||window.oClearImmediate||function(e){window.clearTimeout(e)}
	}()
}
