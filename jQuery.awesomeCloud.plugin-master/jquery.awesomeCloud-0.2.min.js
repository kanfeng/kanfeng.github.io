...
var t="awesomeCloud",
n={size:{grid:8,factor:0,normalize:true},
...
r.prototype={
	create:function(){
	var n=this,r=0,i=null,s=0,o=0,
	    u=t+"TempCheck",a=null,f=false,l=0,c=.1,h=0,p=0,d=0,v=0,m=null,g=null,y=null,b,w,E,S,x;
	...
	this.container.children().each(function(t,r){//获取每个span内容
		m=null;y=null;
		o=parseInt(e(this).attr("data-weight"),10);
		s+=o;
		if(!n.settings.weight.lowest){n.settings.weight.lowest=o}
	...
	this.settings.range=this.settings.weight.highest-this.settings.weight.lowest;
	  if(this.settings.size.normalize===true){
		  this.words.sort(function(e,t){return e[1]-t[1]});
		   for(r=0;r<this.words.length;r++){
			   if(a===null){a=this.words[r][1]}
			   else{if(this.words[r][1]-a>this.settings.weight.average){this.words[r][1]-=(this.words[r][1]-a)/(this.settings.weight.average*.38)+a}
			   }
			}
	  }
	...
	this.bgPixel=this.bctx.getImageData(0,0,1,1).data;
	    ...
	    if(typeof this.settings.options.color!=="function"
...
putWord:function(e,n,r,i){
			...
			if(o){a=this.ctx.measureText(e).width;
...
canFitText:function(e,t,n,r){
	...
	{return false}
	 ...
	 {s=r;
      while(s--)
	    {if(!this.grid[e+i][t+s]){return false}
	    }
	 }
 return true
 },
...
wordcolor:function(e,t,n,r,i){
	var s=null;switch(this.settings.options.color){
		case"gradient":s="rgba("+Math.round(this.settings.color.start.r+this.settings.color.increment.r*(t-this.settings.weight.lowest))+","+Math.round(this.settings.color.start.g+this.settings.color.increment.g*(t-this.settings.weight.lowest))+","+Math.round(this.settings.color.start.b+this.settings.color.increment.b*(t-this.settings.weight.lowest))+","+Math.round(this.settings.color.start.a+this.settings.color.increment.a*(t-this.settings.weight.lowest))+")";break;case"random-dark":s="rgba("+Math.floor(Math.random()*128).toString(10)+","+Math.floor(Math.random()*128).toString(10)+","+Math.floor(Math.random()*128).toString(10)+",1)";break;case"random-light":s="rgba("+Math.floor(Math.random()*128+128).toString(10)+","+Math.floor(Math.random()*128+128).toString(10)+","+Math.floor(Math.random()*128+128).toString(10)+",1)";break;default:if(typeof this.settings.wordColor!=="function"){s="rgba(127,127,127,1)"}else{s=this.settings.wordColor(e,t,n,r,i)}break}
	return s
},
		
updateGrid:function(e,t,n,r,i){
	var s=n,o,
	u=this.ctx.getImageData(e*this.settings.gridSize,t*this.settings.gridSize,n*this.settings.gridSize,r*this.settings.gridSize);
	while(s--){o=r;while(o--){if(!this.isGridEmpty(u,s*this.settings.gridSize,o*this.settings.gridSize,n*this.settings.gridSize,r*this.settings.gridSize,i)){this.grid[e+s][t+o]=false}}}
},
		
isGridEmpty:function(e,t,n,r,i,s){
	var o=this.settings.gridSize,u,a;
	if(!isNaN(this.diffChannel)&&!s)
	 {while(o--){u=this.settings.gridSize;
         while(u--){
	     ...
	     {u=this.settings.gridSize;while(u--)
		   {a=4;
	        while(a--){if(this.getChannelData(e.data,t+o,n+u,r,i,a)!==this.bgPixel[a]){return false}}
		   }
		 }
		}
	return true
...
getTextHeight:function(t){
	var n=e('<span style="font: '+t+'">Hg</span>'),
	r=e('<div style="display: inline-block; width: 1px; height: 0px;"></div>'),
	i=e("<div></div>"),
	s=e("body"),
	o={};
	i.append(n,r);
